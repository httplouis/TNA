  ...
  return (
    <div className="glass-card p-6 text-left mb-6">
      ...
      <p className="text-xs text-slate-500 mb-5">
        Based on your scores. Final recommendations will be confirmed by our team after review.
      </p>
      ...
      {allGood ? (
        <div className="flex items-center gap-3 py-3 px-4 rounded-xl bg-green-500/10 border border-green-500/20">
          ...
          <p className="text-sm text-green-300">
            Your scores are strong across all categories. Advanced or leadership programs are recommended.
          </p>
        </div>
      ) : (
              ...
              <div key={r.category} className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    >
                      {r.level === "Needs Improvement" && <AlertTriangle className="w-3 h-3" />}
                      {r.level}
                    </span>
                  </div>
      ...
      <div className="mt-5 pt-5 border-t border-white/5">
        <p className="text-xs text-slate-500 mb-3">Your scores at a glance:</p>
      ...
      <p className="text-xs text-slate-600 mt-4 italic">
        Note: These are system-generated suggestions. Our team will review and finalise your results before sending them.
      </p>
    </div>
  );
}
...
export default function SuccessClient() {
  ...
  useEffect(() => {
    ...
    if (latest.results && latest.results.length > 0) {
      setLatestResults(latest.results);
    }
  }, []);
    ...
    <div className="min-h-screen bg-[#0c1220] flex flex-col items-center justify-start px-6 py-12 text-center">
      ...
      <div className="relative max-w-xl w-full animate-scale-in">
        ...
        <p className="text-slate-400 leading-relaxed mb-8">
          Your Training Needs Assessment has been successfully submitted. Our team will review your results
          and send you personalised training recommendations.
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Link
            href="/"
            className="flex-1 flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-sm font-medium border border-white/10 text-slate-400 hover:text-white hover:border-white/20 transition-all"
          >
          ...
          <Link
            href="/survey"
            className="flex-1 flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-sm font-semibold bg-[#1d6eb5] hover:bg-[#1a5fa0] text-white shadow-lg shadow-blue-900/30 transition-all hover:-translate-y-0.5"
          >
        ...
        <p className="text-xs text-slate-600 mt-6">
          Informatics Holdings Philippines &middot; TNA Portal
        </p>
      </div>
    </div>
  );
