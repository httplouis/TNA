...
export default function ReviewStep({ info, responses, openAnswers, onBack, onSubmit, submitting }: Props) {
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <p className="text-slate-400 text-sm mb-8">Please verify your information before submitting.</p>
      ...
      <div className="glass-card p-5 mb-5">
            ...
            return (
              <div key={sec.sectionNumber}>
                <div className="flex justify-between items-center text-xs mb-1">
                  <span className="text-slate-300">Section {sec.sectionNumber}: {sec.title}</span>
                  <span className="text-slate-500">{secAnswered}/{secQs.length}</span>
                </div>
        ...
        <div className="mt-4 pt-4 border-t border-white/5 flex justify-between text-sm">
          ...
          <span className="font-bold text-white">{answered} / {totalQs}</span>
        </div>
      </div>
      ...
      {(openAnswers.tasksPerformed || openAnswers.trainingGoals) && (
        <div className="glass-card p-5 mb-5">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Section 7 Responses</h3>
          {openAnswers.tasksPerformed && (
            <div className="mb-3">
              <p className="text-xs text-slate-500 mb-1">Tasks performed:</p>
              <p className="text-sm text-slate-300">{openAnswers.tasksPerformed}</p>
            </div>
          )}
          {openAnswers.trainingGoals && (
            <div>
              <p className="text-xs text-slate-500 mb-1">Training goals:</p>
              <p className="text-sm text-slate-300">{openAnswers.trainingGoals}</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex gap-3">
        <button onClick={onBack} className="px-5 py-3 rounded-xl text-sm font-medium border border-white/10 text-slate-400 hover:text-white transition-all">
          Edit Answers
        </button>
        <button
          ...
          onClick={onSubmit}
          disabled={submitting}
          className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm bg-[#1d6eb5] hover:bg-[#1a5fa0] text-white shadow-xl shadow-blue-900/30 transition-all hover:-translate-y-0.5 disabled:opacity-60"
        >
          {submitting ? (
